...
| **Testing / maintainability** | `backend/tests/*.test.ts` (+ JWT + auth/password + quiz validators), `frontend/tests/*.test.tsx` (a11y). **GitHub Actions CI** builds & runs tests on push. Run `npm test` in each package. |
